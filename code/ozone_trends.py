...
for yy, year in enumerate(years):
    ...
    ds3.index = pd.to_datetime(hourly[start_year:end_year].index,format='%d/%m/%Y')
    ds3 = ds3.resample('M').max()[0]
    ...
    for i in range(len(ds)):
        ...
        #plt.ylim(13,44)
    plt.tight_layout()
    plt.savefig(savepath+'/ozonea.png')
    plt.close()
    ...
    plt.legend()
    plt.ylabel('$O_3$ (ppbv)')
    plt.savefig(savepath+'/ozone.png')
    plt.close()
    ...
    lsk = str(np.round(z[0]*1e3,2))
    hold.append(lsk)
    print(end_year, str(np.round(z[0]*1e3,2)))